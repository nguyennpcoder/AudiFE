...
type SalesConsoleLayoutProps = {
  title?: string;
  children: React.ReactNode;
};

const SalesConsoleLayout: React.FC<SalesConsoleLayoutProps> = ({ title = 'Sales Console', children }) => {
  return (
    <div style={{ background: 'linear-gradient(180deg, #f6f7fb 0%, #eef2f7 60%, #e9eef5 100%)', minHeight: '100vh' }}>
      <div
      ...
      >
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 18, letterSpacing: 0.5 }}>Audi Sales</div>
          </div>
          <div style={{ opacity: 0.9, fontSize: 14 }}>Deals & Orders</div>
        </div>
      </div>
      ...
      <div className="admin-animate-bottom" style={{ maxWidth: 1200, margin: '24px auto 0', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <div style={{ color: '#6b7280', fontSize: 12, letterSpacing: 0.3 }}>Sales / Console</div>
            <h1 style={{ margin: 4, fontSize: 28, fontWeight: 800, color: '#0f172a' }}>{title}</h1>
            <div style={{ color: '#6b7280', marginTop: 6, fontSize: 14 }}>Quản lý đơn hàng, khách hàng và pipeline</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="support-primary-btn" title="Tạo đơn mới">
...
export default SalesConsoleLayout;
